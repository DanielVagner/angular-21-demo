import { Component, signal, resource, ResourceStatus, ChangeDetectionStrategy, inject } from '@angular/core';
...
@Component({
  ...
  template: `
    <div class="demo-page">
      ...
      <div class="live-demo">
        ...
        <div class="controls">
          ...
          <div class="id-controls">
            @for (id of [1, 2, 3, 4, 5]; track id) {
              <button class="btn" [class.btn-primary]="userId() === id" [class.btn-ghost]="userId() !== id" (click)="userId.set(id)">
                User {{ id }}
              </button>
            }
        ...
        <div class="status-row">
          ...
          @switch (postsResource.status()) {
            ...
            @case (ResourceStatus.Resolved)  { <span class="status-chip status-chip-success">✅ Resolved</span> }
            @case (ResourceStatus.Error)     { <span class="status-chip status-chip-error">❌ Error</span> }
            @case (ResourceStatus.Idle)      { <span class="status-chip status-chip-idle">💤 Idle</span> }
          }
        ...
        @if (postsResource.error()) {
          <div class="error-msg" style="margin-top:0.75rem">{{ postsResource.error() }}</div>
        }

        <div class="posts-list">
          @if (postsResource.isLoading() && !postsResource.value()) {
      ...
      <pre class="code-block">userId = <span style="color:#d2a8ff">signal</span>(1);

postsResource = <span style="color:#d2a8ff">resource</span>(&#123;
  // params() — reaktivní vstup; při změně se loader znovu spustí
  params: () => (&#123; userId: this.userId() &#125;),

  // loader — async funkce která vrací data
  loader: async (&#123; params, abortSignal &#125;) => &#123;
    const res = await fetch(
      `/api/posts?userId=$&#123;params.userId&#125;`,
      &#123; signal: abortSignal &#125;  // ← automatický abort
    );
...
// Přístup ke stavům:
postsResource.status()    // Idle | Loading | Resolved | Error | Reloading
postsResource.value()     // T | undefined
postsResource.error()     // unknown
postsResource.isLoading() // boolean

// Manuální reload:
postsResource.reload();

// Lokální update bez HTTP volání:
postsResource.update(posts => [...posts, newPost]);</pre>

      <!-- ResourceStatus přehled -->
      <div class="section-title">ResourceStatus — stavový diagram</div>
      <div class="status-diagram">
        @for (s of statusStates; track s.name) {
          <div class="state-card" [class]="'state-' + s.color">
  ...
  styles: [`
    ...
    .post-card {
      display:flex; gap:0.75rem; background:var(--surface); border:1px solid var(--border);
      border-radius:8px; padding:0.75rem; transition:opacity 0.3s;
      &.fading { opacity:0.5; }
    }
    ...
    .post-skeleton {
      height:60px; background:linear-gradient(90deg, var(--surface) 25%, var(--card) 50%, var(--surface) 75%);
      background-size:200% 100%; animation:shimmer 1.5s infinite; border-radius:8px; border:1px solid var(--border);
    }
...
export class ResourceDemoComponent {
  ...
  postsResource = resource<Post[], { userId: number }>({
    params: () => ({ userId: this.userId() }),
    loader: async ({ params, abortSignal }) => {
      const res = await fetch(
  ...
  statusStates = [
    { name: 'Idle',       color: 'idle',     desc: 'request() vrátilo undefined, žádné volání' },
